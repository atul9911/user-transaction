...
class RoutesHandler {
  ...
  public static void createUserHandler(HttpServerExchange httpServerExchange) throws Exception {
    try {
      ...
      httpServerExchange.getResponseSender()
          .send(CommonUtil.objectMapper.writeValueAsString(apiResponse));
    }
  }
  ...
  public static void initiateTransaction(HttpServerExchange httpServerExchange) throws Exception {
    try {
      httpServerExchange.startBlocking();
      TransactionPojo transactionPojo = CommonUtil
          .getClassObjectFromStream(IOUtils
                  .toString(httpServerExchange.getInputStream(), StandardCharsets.UTF_8.name()),
  ...
  public static void makeTransaction(HttpServerExchange httpServerExchange) throws Exception {
    try {
      ...
      TransactionPojo transactionPojo = transactionService
          .doTransaction(Integer.valueOf(transactionId));
